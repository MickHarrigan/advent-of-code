...
fn parse_schematic(contents: String) -> usize {
    ...
    let things: Vec<Vec<Item>> = contents
        .lines()
        .zip(0u32..)
        .map(|(line, line_num)| {
            ...
            for (ind, ch) in line.char_indices() {
                match ch {
                    ...
                    '#' | '%' | '&' | '*' | '+' | '-' | '/' | '=' | '@' | '$' => {
                        ...
                        list.push(Item::Symbol(Location {
                            x: ind as u32,
                            y: line_num,
                        }));
                    }
        ...
        for lhs in win[0].iter() {
            for rhs in win[1].iter() {
                ...
                if let Item::Number(num, reg) = lhs {
                    if let Item::Symbol(loc) = rhs {
                        if reg.contains(loc) {
                            sum += num;
                        }
                    }
                // given rhs is num and lhs is a symbol
                } else if let Item::Symbol(loc) = lhs {
        ...
        if list.len() >= 2 {
            list.windows(2).for_each(|win| {
                ...
                if let Item::Number(num, reg) = &win[0] {
                    if let Item::Symbol(loc) = &win[1] {
                        if reg.contains(&loc) {
                            sum += num;
                        }
                    }
                // given rhs is num and lhs is a symbol
                } else if let Item::Symbol(loc) = &win[0] {
