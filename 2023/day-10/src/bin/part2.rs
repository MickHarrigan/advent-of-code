...
fn solve(data: (Vec<Vec<Pipes>>, (usize, usize))) -> usize {
    ...
    // extra start needed at the front for the shoelace
    corners.push(start);
    ...
    let area: isize = corners
        .windows(2)
        .map(|window| (window[0].0 * window[1].1) as isize - (window[0].1 * window[1].0) as isize)
        .sum();

    (area.abs() / 2) as usize - corners.len() / 2 + 1
}
